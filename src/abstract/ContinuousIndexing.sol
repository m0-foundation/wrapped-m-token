...
abstract contract ContinuousIndexing is IContinuousIndexing {
    ...
    function _updateRate(uint32 newRate_) internal returns (uint128 currentIndex_) {
        // NOTE: `currentIndex()` depends on `_latestRate`, so only update it after getting `currentIndex()`.
        latestIndex = currentIndex_ = currentIndex();
        _latestRate = newRate_;
        latestUpdateTimestamp = uint40(block.timestamp);

        emit RateUpdated(currentIndex_, newRate_);
    }
