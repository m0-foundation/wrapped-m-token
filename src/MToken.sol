...
/**
 * @title  Mock MToken (admin can mint and set the earner rate).
 * @author M^0 Labs
...
contract MToken is IMToken, ContinuousIndexing, ERC20Extended {
    ...
    /// @dev Modifier to check if the caller is the minter.
    modifier onlyMinter() {
    ...
    constructor() ContinuousIndexing() ERC20Extended("Mock M by M^0", "M", 6) {
        minter = msg.sender;
    }
    ...
    function mint(address account_, uint256 amount_) external onlyMinter {
        ...
        _revertIfInvalidRecipient(account_);
        ...
        if (_balances[account_].isEarning) {
            ...
            _addEarningAmount(account_, _getPrincipalAmountRoundedDown(safeAmount_));
        } else {
            _addNonEarningAmount(account_, safeAmount_);
        }
    }

    /// @inheritdoc IMToken
    function startEarning(address account_) external onlyMinter {
    ...
    /// @inheritdoc IMToken
    function setEarnerRate(uint32 rate_) external onlyMinter {
        _updateRate(rate_);
    }
