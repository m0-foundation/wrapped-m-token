...
abstract contract AccountFundsDistribution {
    ...
    function _distribute(address vault_, uint256 distributable_) internal {
        ...
        if (totalShares_ == 0) revert Common.ZeroTotalShares(); // If there are no shares for this vault, then revert.
    ...
    function _addShares(address vault_, uint256 distributable_, address account_, uint256 amount_) internal {
        ...
        _correctionPoints[vault_][account_] -= Common.toInt256(_pointsPerShares[vault_] * amount_);
    ...
    function _getShares(address vault_, address account_) internal view returns (uint256 shares_) {
        return _shares[vault_][account_];
    }

    /// @dev Returns the total participating shares of `vault_`.
    function _getTotalShares(address vault_) internal view returns (uint256 totalShares_) {
        return _totalShares[vault_];
    }
}
